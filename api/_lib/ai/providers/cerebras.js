/**
 * ai/providers/cerebras.js
 *
 * Cerebras Llama-3.3-70B adapter — OpenAI-compatible chat/completions.
 *
 * Architecture: FULL-P1-I · CEO Phase-0 approved.
 *
 * IMPORTANT — RATE LIMIT: Cerebras free tier is 5 RPM GLOBALLY
 * (CEO Phase-0 verification, NOT 30 RPM as some older docs suggest).
 * This adapter is prodEnabled: false — exercised ONLY in dev/QA via
 * the owner debug-header  X-PFC-Force-Provider: cerebras.
 *
 * Signature:
...
export default async function cerebrasAdapter({ systemPrompt, messages, opts = {} }, apiKey) {
  ...
  try {
    ...
    console.error('[ai:cerebras]', reason, 'status=0 code=fetch_error');
  ...
  if (!res.ok) {
    ...
    console.error('[ai:cerebras]', reason, 'status=' + status + ' code=' + errCode);
    return {
      ok: false,
      reason,
      status,
  ...
  try {
    ...
    console.error('[ai:cerebras]', 'BAD_RESPONSE', 'status=' + status + ' code=json_parse_error');
    return { ok: false, reason: 'BAD_RESPONSE', status, retryAfterSec: null };
  }

  const reply = data?.choices?.[0]?.message?.content;
  if (typeof reply !== 'string' || !reply.trim()) {
    console.error('[ai:cerebras]', 'BAD_RESPONSE', 'status=' + status + ' code=empty_content');
    return { ok: false, reason: 'BAD_RESPONSE', status, retryAfterSec: null };
  }
